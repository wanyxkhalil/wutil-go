...
| api        | 控制器模块，负责定义资源路由、参数验证、请求处理逻辑                                                        |
| service    | 服务模块，包含核心业务逻辑的实现                                                                            |
| dao        | 数据访问模块，负责与数据库进行交互，执行数据存储、查询、更新和删除操作。以 single record 模式保证数据完整性 |
| record     | 数据访问模块，负责与数据库进行交互，执行数据存储、查询、更新和删除操作。以 single record 模式保证数据完整性 |
| middleware | 中间件模块，处理请求验证、日志记录、错误处理等通用逻辑                                                      |
| sdk        | SDK 客户端模块，封装与外部服务交互的客户端逻辑                                                              |
